  ...
  <div class="w-full relative display flex flex-col justify-center items-center p-4 shadow-lg rounded-lg bg-white">
    ...
    <div class="w-full relative display flex justify-center items-center">
      ...
      <img
        ...
        alt={salesPoint.name}
      />
      <p class="absolute text-white tracking-widest text-xl px-10 text-center font-bold z-10">
        {salesPoint.name}
      </p>
    </div>
  
    <!-- Card Details (Address, Phone Numbers, Working Hours) -->
    <div class="w-full mt-4 text-center space-y-2"> <!-- Add space-y-2 for vertical spacing -->
      <p class="text-sm font-semibold">{salesPoint.address}</p>
      <p class="text-sm text-gray-500">
        Telefon: 
        {#each salesPoint.phoneNumbers as phoneNumber, i}
          {phoneNumber}{#if i < salesPoint.phoneNumbers.length - 1}, {/if} <!-- Space after the comma -->
          {#if i < salesPoint.phoneNumbers.length - 1} {/if} <!-- Add a space after the comma -->
        {/each}
      </p>
